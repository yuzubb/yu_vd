...
class VendingCog(commands.Cog):
    ...
    @vm.command(name="作成", description="新しい自販機を作成します")
    @is_allowed()
    ...
    async def create(self, interaction: discord.Interaction, id: str, name: str):
        ...
        embed.add_field(name="次のステップ", value="`/自販機 商品追加` で商品を登録し、`/自販機 パネル設置` でパネルを設置してください。", inline=False)
        await interaction.response.send_message(embed=embed, ephemeral=True)

    @vm.command(name="削除", description="自販機のデータを削除します")
    @is_allowed()
    ...
    @vm.command(name="パネル設置", description="自販機パネルをこのチャンネルに設置します")
    @is_allowed()
    @app_commands.autocomplete(id=vending_autocomplete)
    ...
    async def panel_set(self, interaction: discord.Interaction, id: str):
        ...
        if not data.get("products"):
            return await interaction.response.send_message("❌ 商品が登録されていません。先に `/自販機 商品追加` で商品を追加してください。", ephemeral=True)
    ...
    @vm.command(name="パネル削除", description="設置された自販機パネルを削除します")
    @is_allowed()
    @app_commands.autocomplete(id=vending_autocomplete)
    ...
    @vm.command(name="パネル更新", description="パネルを最新の情報に更新します")
    @is_allowed()
    @app_commands.autocomplete(id=vending_autocomplete)
    @app_commands.describe(id="自販機のID")
    ...
    @vm.command(name="商品追加", description="自販機に商品を追加します")
    @is_allowed()
    @app_commands.autocomplete(id=vending_autocomplete)
    ...
    @vm.command(name="商品削除", description="商品を削除します")
    @is_allowed()
    ...
    @vm.command(name="商品編集", description="商品の情報を変更します")
    @is_allowed()
    @app_commands.autocomplete(id=vending_autocomplete, product_id=product_autocomplete)
    ...
    @vm.command(name="在庫追加", description="商品に在庫を追加します(在庫無限の場合は現在の在庫をDMに送信してから追加します)")
    @is_allowed()
    @app_commands.autocomplete(id=vending_autocomplete, product_id=product_autocomplete)
    ...
    @vm.command(name="在庫引き出し", description="商品の在庫を引き出します")
    @is_allowed()
    @app_commands.autocomplete(id=vending_autocomplete, product_id=product_autocomplete)
    @app_commands.describe(
        id="自販機のID",
    ...
    @vm.command(name="通知設定", description="在庫追加通知チャンネルを設定します")
    @is_allowed()
    @app_commands.autocomplete(id=vending_autocomplete)
    ...
    @vm.command(name="通知削除", description="在庫追加通知の設定を削除します")
    @is_allowed()
    @app_commands.autocomplete(id=vending_autocomplete)
    @app_commands.describe(id="自販機のID")
    ...
    @vm.command(name="ログ設定", description="購入ログを送信するチャンネルを設定します")
    @is_allowed()
    @app_commands.autocomplete(id=vending_autocomplete)
    ...
    @vm.command(name="ログ削除", description="購入ログチャンネルの設定を削除します")
    @is_allowed()
    @app_commands.autocomplete(id=vending_autocomplete)
    @app_commands.describe(id="自販機のID")
