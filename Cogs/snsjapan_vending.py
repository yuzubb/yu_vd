...
# ─── 設定 ──────────────────────────────────────────────────────────────────────
SNSJ_API_URL  = "https://snsjapan.site/api/v2"
SNSJ_API_KEY  = os.getenv("SNSJAPAN_API_KEY", "")

SNSJ_ORDER_FILE   = "data/snsjapan_orders.json"
SNSJ_MENU_FILE    = "data/snsjapan_menu.json"
PAYPAY_FILE       = "paypay_data.json"   # vending.py と共有
SNSJ_VENDING_FILE = "data/snsjapan_vending_data.json"
SNSJ_PENDING_FILE = "data/paypay_pending_snsjapan.json"

JST = pytz.timezone("Asia/Tokyo")

# snsjapan.site はJPY建てのため換算レートは 1.0
# rate が USD建ての場合は変更してください
SNSJ_RATE_UNIT  = float(os.getenv("SNSJAPAN_RATE_UNIT", "155.0"))  # USD→JPY換算レート（APIがUSD建てのため）
SNSJ_RATE_PER   = int(os.getenv("SNSJAPAN_RATE_PER", "1000"))     # rateが何件単位か
...
DEFAULT_MENU_TEMPLATES = [
    # Instagram
    {"category": "instagram", "keywords": ["instagram followers",   "instagramフォロワー",   "フォロワー instagram"], "keyword": "instagram followers", "name": "Instagramフォロワー",     "margin_rate": 3.0, "min_qty": 100,  "max_qty": 10000},
    {"category": "instagram", "keywords": ["instagram likes",       "instagramいいね",       "いいね instagram"],     "keyword": "instagram likes",     "name": "Instagramいいね",         "margin_rate": 3.0, "min_qty": 50,   "max_qty": 5000},
    {"category": "instagram", "keywords": ["instagram views",       "instagram動画再生",     "再生 instagram"],       "keyword": "instagram views",     "name": "Instagram動画再生数",     "margin_rate": 3.0, "min_qty": 1000, "max_qty": 100000},
    # TikTok
    {"category": "tiktok",    "keywords": ["tiktok followers",      "tiktokフォロワー",      "フォロワー tiktok"],    "keyword": "tiktok followers",    "name": "TikTokフォロワー",        "margin_rate": 3.0, "min_qty": 100,  "max_qty": 10000},
    {"category": "tiktok",    "keywords": ["tiktok likes",          "tiktokいいね",          "いいね tiktok"],        "keyword": "tiktok likes",        "name": "TikTokいいね",            "margin_rate": 3.0, "min_qty": 100,  "max_qty": 10000},
    {"category": "tiktok",    "keywords": ["tiktok views",          "tiktok動画再生",        "再生 tiktok"],          "keyword": "tiktok views",        "name": "TikTok動画再生数",        "margin_rate": 3.0, "min_qty": 1000, "max_qty": 500000},
    # X (Twitter)
    {"category": "twitter",   "keywords": ["twitter followers",     "twitterフォロワー",     "xフォロワー"],          "keyword": "twitter followers",   "name": "Xフォロワー",             "margin_rate": 3.0, "min_qty": 100,  "max_qty": 10000},
    {"category": "twitter",   "keywords": ["twitter likes",         "twitterいいね",         "xいいね"],              "keyword": "twitter likes",       "name": "Xいいね",                 "margin_rate": 3.0, "min_qty": 50,   "max_qty": 5000},
    # YouTube
    {"category": "youtube",   "keywords": ["youtube subscribers",   "youtubeチャンネル登録", "チャンネル登録"],       "keyword": "youtube subscribers", "name": "YouTubeチャンネル登録",   "margin_rate": 3.0, "min_qty": 100,  "max_qty": 5000},
    {"category": "youtube",   "keywords": ["youtube views",         "youtube再生",           "再生回数 youtube"],     "keyword": "youtube views",       "name": "YouTube再生回数",         "margin_rate": 3.0, "min_qty": 1000, "max_qty": 500000},
    # Threads
    {"category": "threads",   "keywords": ["threads followers",     "threadsフォロワー",     "フォロワー threads"],   "keyword": "threads followers",   "name": "Threadsフォロワー",       "margin_rate": 3.5, "min_qty": 100,  "max_qty": 5000},
]
...
def load_snsj_menu() -> list:
    _ensure_data_dir()
    if os.path.exists(SNSJ_MENU_FILE):
        with open(SNSJ_MENU_FILE, "r", encoding="utf-8") as f:
            try:
                return json.load(f)
            except:
                return []
...
def save_snsj_menu(data: list):
    _ensure_data_dir()
    with open(SNSJ_MENU_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=4, ensure_ascii=False)

def load_vending_data() -> dict:
    return load_json(SNSJ_VENDING_FILE)

def save_vending_data(data: dict):
    save_json(SNSJ_VENDING_FILE, data)

def load_paypay_data() -> dict:
    return load_json(PAYPAY_FILE)

# ─── snsjapan.site API ────────────────────────────────────────────────────────
async def snsj_request(payload: dict) -> dict:
    payload["key"] = SNSJ_API_KEY
    async with aiohttp.ClientSession() as session:
        async with session.post(SNSJ_API_URL, data=payload) as resp:
...
async def snsj_add_order(service_id: int, link: str, quantity: int) -> dict:
    return await snsj_request({"action": "add", "service": service_id, "link": link, "quantity": quantity})

async def snsj_get_status(order_id) -> dict:
    return await snsj_request({"action": "status", "order": order_id})

async def snsj_get_balance() -> dict:
    return await snsj_request({"action": "balance"})

async def snsj_get_services() -> list:
    result = await snsj_request({"action": "services"})
    return result if isinstance(result, list) else []

def _calc_price_jpy(rate_str, margin_rate: float) -> float:
    """snsjapanのrate（RATE_UNIT / RATE_PER件）を円換算して利益マージンを乗せ、1件あたりの円価格を返す"""
    try:
        rate = float(rate_str)
        cost_jpy_per_1 = rate * SNSJ_RATE_UNIT / SNSJ_RATE_PER
        return round(cost_jpy_per_1 * margin_rate, 2)
    except (TypeError, ValueError):
        return 0.0

def _match_service(services: list, keyword) -> dict | None:
    keywords = keyword if isinstance(keyword, list) else [keyword]
    for svc in services:
        svc_name = svc.get("name", "").lower()
        for kw in keywords:
            kw_lower = kw.lower()
            if kw_lower in svc_name:
                return svc
            words = kw_lower.split()
            if len(words) > 1 and all(w in svc_name for w in words):
                return svc
    return None
...
def _build_paypay_embed(sender, amount, money, money_light,
    ...
    embed.set_footer(text="受け取り済み" if received else "SNSジャパン自販機")
    return embed


class SnsjPayPayReceiveView(ui.View):
    ...
    @ui.button(label="受け取る", style=discord.ButtonStyle.danger, custom_id="snsj_paypay_receive_btn")
    async def receive_btn(self, interaction: discord.Interaction, button: ui.Button):
        ...
        if not info:
            return await interaction.followup.send(
                "❌ オーナーのPayPayアカウントが未登録です。`/snsj_paypay登録` で登録してください。", ephemeral=True
            )
...
# ─── 購入フロー UI ────────────────────────────────────────────────────────────
class SnsjQuantityModal(ui.Modal, title="購入情報入力"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.set_footer(text="SNSジャパン自販機")

        view = SnsjPurchaseConfirmView(
...
class SnsjPayPayModal(ui.Modal, title="PayPay決済"):
...
class SnsjPurchaseConfirmView(ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: ui.Button):
        if self.total_price == 0:
            ...
            await interaction.response.send_modal(SnsjPayPayModal(self))
    ...
    async def process_purchase(self, interaction: discord.Interaction, pay_link):
        ...
        mi       = self.menu_item
        snsj_resp = await snsj_add_order(mi["service_id"], self.sns_url, self.quantity)

        if "error" in snsj_resp or "order" not in snsj_resp:
            return await interaction.followup.send(
                f"❌ SNSジャパン注文に失敗しました。\n```{snsj_resp.get('error', str(snsj_resp))}```", ephemeral=True
            )

        snsj_order_id = snsj_resp["order"]

        orders = load_json(SNSJ_ORDER_FILE)
        orders[str(snsj_order_id)] = {
            ...
            "snsj_order_id":   snsj_order_id,
            "status":          "Pending",
            "created_at":      datetime.datetime.now(JST).strftime("%Y/%m/%d %H:%M:%S"),
        }
        save_json(SNSJ_ORDER_FILE, orders)
        ...
        embed.add_field(name="注文ID",   value=f"```{snsj_order_id}```",      inline=True)
        embed.set_footer(text="注文IDは「状況確認」ボタンで使えます。 | SNSジャパン自販機")
        ...
        try:
            ...
            if log_ch_id:
                ...
                if log_ch:
                    ...
                    log_embed.add_field(name="OrderID", value=str(snsj_order_id),              inline=False)
                    log_embed.set_footer(text="SNSジャパン自販機")
                    await log_ch.send(embed=log_embed)
        except Exception as e:
            print(f"[snsj_log_channel] error: {e}")
        ...
        try:
            ...
            print(f"[snsj_role_grant] error: {e}")
        ...
        try:
            ...
            dm_embed.add_field(name="注文ID",       value=f"```{snsj_order_id}```",                                                                                              inline=True)
            dm_embed.set_footer(text="SNSジャパン自販機")
        ...
        try:
            ...
            if owner_user:
                ...
                owner_embed.add_field(name="注文ID",   value=f"```{snsj_order_id}```",                            inline=True)
                owner_embed.add_field(name="サーバー", value=f"```{interaction.guild.name if interaction.guild else 'DM'}```", inline=True)
                owner_embed.set_footer(text="SNSジャパン自販機")
...
# ─── パネル UI ────────────────────────────────────────────────────────────────
class SnsjCategorySelect(ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        items    = [m for m in load_snsj_menu() if m.get("vm_id") == self.vm_id and m.get("category") == category]
        ...
        await interaction.followup.send(
            embed=embed,
            view=SnsjServiceSelectView(self.vm_id, items, self.bot),
            ephemeral=True,
        )


class SnsjServiceSelectView(ui.View):
    def __init__(self, vm_id, items, bot):
        super().__init__(timeout=120)
        self.add_item(SnsjServiceSelectInner(vm_id, items, bot))


class SnsjServiceSelectInner(ui.Select):
    def __init__(self, vm_id, items, bot):
        self.vm_id = vm_id
        self.items = items
        self.bot   = bot
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_modal(SnsjQuantityModal(item, self.vm_id, self.bot))


class SnsjOrderCheckModal(ui.Modal, title="注文状況の照会"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        result = await snsj_get_status(order_id)
...
class SnsjStatusButton(ui.Button):
    def __init__(self):
        super().__init__(label="状況確認", style=discord.ButtonStyle.secondary, custom_id="snsj_status_btn", row=1)

    async def callback(self, interaction: discord.Interaction):
        await interaction.response.send_modal(SnsjOrderCheckModal())


class SnsjHistoryView(ui.View):
    ITEMS_PER_PAGE = 5
...
class SnsjHistoryButton(ui.Button):
    def __init__(self):
        super().__init__(label="購入履歴", style=discord.ButtonStyle.primary, custom_id="snsj_history_btn", row=1)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view  = SnsjHistoryView(records, interaction.user.id)
        embed = view.build_embed()
        await interaction.response.send_message(embed=embed, view=view, ephemeral=True)


class SnsjPanelView(ui.View):
    def __init__(self, vm_id: str, bot: commands.Bot):
        super().__init__(timeout=None)
        self.vm_id = vm_id
        self.bot   = bot
        self.add_item(SnsjCategorySelect(vm_id, bot))
        self.add_item(SnsjStatusButton())
        self.add_item(SnsjHistoryButton())


# ─── 売上集計 ─────────────────────────────────────────────────────────────────
class SnsjSalesPagingView(ui.View):
    ITEMS_PER_PAGE = 8
    ...
    def _overview_embed(self) -> discord.Embed:
        ...
        embed.set_footer(text="1/3 ページ　|　SNSジャパン自販機")
        return embed
    ...
    def _hourly_embed(self) -> discord.Embed:
        ...
        embed.set_footer(text="2/3 ページ　|　SNSジャパン自販機")
        return embed
    ...
    def _buyers_embed(self) -> discord.Embed:
        ...
        embed.set_footer(text=f"3/3 ページ ({self.buyer_page+1}/{self.buyer_total})　|　SNSジャパン自販機")
        return embed
...
# ─── Cog ─────────────────────────────────────────────────────────────────────
class SnsjVendingCog(commands.Cog):
    ...
    async def cog_load(self):
        ...
        for vm_id in vending_data.keys():
            self.bot.add_view(SnsjPanelView(vm_id, self.bot))
        self.bot.add_view(SnsjPayPayReceiveView(
    ...
    async def panel_refresh(self):
        ...
        for vm_id, vm in vending_data.items():
            ...
            try:
                ...
                menu_items = [m for m in load_snsj_menu() if m.get("vm_id") == vm_id]
                old_embed  = msg.embeds[0] if msg.embeds else None
                title      = old_embed.title       if old_embed else "24時間SNS自販機"
                ...
                view = SnsjPanelView(vm_id, self.bot)
                await msg.edit(embed=embed, view=view)
            except Exception as e:
                print(f"[snsj_panel_refresh] vm_id={vm_id} error: {e}")
    ...
    async def order_status_check(self):
        ...
        for order_id, rec in list(orders.items()):
            ...
            if prev_status in NOTIFY_STATUSES:
                continue
            try:
                result     = await snsj_get_status(int(order_id))
                ...
                try:
                    ...
                    print(f"[snsj_order_status_check] user dm error: {e}")
            except Exception as e:
                print(f"[snsj_order_status_check] order={order_id} error: {e}")
    ...
    async def on_message(self, message: discord.Message):
        ...
        vd = load_vending_data()
        snsj_detect_channels = {vm.get("paypay_detect_channel_id") for vm in vd.values() if vm.get("paypay_detect_channel_id")}
        if message.channel.id not in snsj_detect_channels:
            return
        ...
        for link in links:
            try:
                ...
                print(f"[snsj_on_message paypay] error: {e}")
    ...
    async def _service_ac(self, interaction: discord.Interaction, current: str):
        ...
        return [
            ...
            for m in load_snsj_menu()
            if m.get("vm_id") == vm_id and current.lower() in m["name"].lower()
        ][:25]

    # ─── コマンド群 ────────────────────────────────────────────────────────────
    @app_commands.command(name="snsj自販機作成", description="SNSジャパン自販機を新規作成します（人気サービスを自動登録）")
    @is_allowed()
    ...
    async def vm_create(
        ...
        lines_msg = [f"✅ SNSジャパン自販機「{name}」を作成しました。", f"**自販機ID:** `{vm_id}`"]
        if not has_paypay:
            lines_msg.append("⚠️ PayPayアカウントが未登録です。`/snsj_paypay登録` を先に実行してください。")

        if auto_menu == "no":
            lines_msg.append("\n💡 `/snsjサービス追加` または `/snsj自動メニュー追加` でサービスを登録してください。")
            return await interaction.followup.send("\n".join(lines_msg), ephemeral=True)

        await interaction.followup.send("\n".join(lines_msg) + "\n\n⏳ サービスを取得して初期メニューを自動登録中...", ephemeral=True)

        services = await snsj_get_services()
        if not services:
            await interaction.followup.send(
                "⚠️ サービスの取得に失敗しました。APIキーを確認するか、"
                "`/snsj自動メニュー追加` で後から自動登録できます。",
                ephemeral=True,
            )
            return
        ...
        result_embed.add_field(
            name="次のステップ",
            value="`/snsjパネル設置` でパネルを設置するとすぐに販売開始できます。\n"
                  "価格は `/snsjサービス削除` → `/snsjサービス追加` で個別に変更できます。",
            inline=False,
        )
        result_embed.set_footer(text=f"マージン倍率: {margin_rate}x | SNSジャパン自販機")
        await interaction.followup.send(embed=result_embed, ephemeral=True)

    @app_commands.command(name="snsjパネル設置", description="SNSジャパン自販機パネルをこのチャンネルに設置します")
    @is_allowed()
    ...
    async def snsj_setup(
        self, interaction: discord.Interaction,
        vm_id: str,
        title: str = "24時間SNS自販機",
        ...
        for cat in SNS_CATEGORIES:
            ...
            embed.add_field(name=cat["label"], value="\n".join(lines), inline=False)
        if not menu_items:
            embed.description += "\n\n**/snsjサービス追加** でサービスを登録してください。"

        view = SnsjPanelView(vm_id, self.bot)
        self.bot.add_view(view)
        await interaction.response.send_message("✅ SNSジャパン自販機パネルを設置しました。", ephemeral=True)
    ...
    @app_commands.command(name="snsjサービス追加", description="SNSジャパンサービスを自販機に追加します")
    @is_allowed()
    @app_commands.autocomplete(vm_id=_vm_id_ac)
    @app_commands.describe(
        vm_id="自販機ID",
        category="SNSカテゴリー",
        service_id="サービスID（/snsjサービス検索で確認）",
        name="表示名（例: Instagramフォロワー）",
        price_per_unit="1個あたりの価格（円）",
        min_qty="最小購入数",
        max_qty="最大購入数",
    )
    @app_commands.choices(category=CATEGORY_CHOICES)
    async def snsj_add_service(
        self, interaction: discord.Interaction,
        ...
        await interaction.response.send_message(
            ...
            f"`/snsjパネル設置` でパネルを再設置すると反映されます。",
            ephemeral=True,
        )

    @app_commands.command(name="snsjサービス削除", description="SNSジャパンサービスをメニューから削除します")
    @is_allowed()
    @app_commands.autocomplete(vm_id=_vm_id_ac, service_id=_service_ac)
    @app_commands.describe(vm_id="自販機ID", service_id="削除するサービスID")
    async def snsj_remove_service(self, interaction: discord.Interaction, vm_id: str, service_id: str):
        vending_data = load_vending_data()
        vm = vending_data.get(vm_id)
        if not vm or vm.get("owner_id") != str(interaction.user.id):
            return await interaction.response.send_message("❌ 指定された自販機が見つかりません。", ephemeral=True)
        ...
        save_snsj_menu(new_menu)
        await interaction.response.send_message(f"✅ サービスID {service_id} を削除しました。", ephemeral=True)

    @app_commands.command(name="snsjサービス一覧", description="登録済みのSNSジャパンサービスを一覧表示します")
    @is_allowed()
    @app_commands.autocomplete(vm_id=_vm_id_ac)
    @app_commands.describe(vm_id="自販機（省略で全件）")
    async def snsj_list_services(self, interaction: discord.Interaction, vm_id: str = None):
        menu = load_snsj_menu()
        if vm_id:
        ...
        embed = discord.Embed(title="📋 SNSジャパンサービス一覧", color=discord.Color.blurple(), timestamp=discord.utils.utcnow())
        embed.set_footer(text="SNSジャパン自販機")
        for cat in SNS_CATEGORIES:
    ...
    @app_commands.command(name="snsj実績チャンネル設定", description="SNSジャパン購入ログを送信するチャンネルを設定します")
    @is_allowed()
    @app_commands.autocomplete(vm_id=_vm_id_ac)
    ...
    @app_commands.command(name="snsjpaypay検知チャンネル設定", description="SNSジャパン用PayPayリンク自動検知チャンネルを設定します")
    @is_allowed()
    @app_commands.autocomplete(vm_id=_vm_id_ac)
    ...
    @app_commands.command(name="snsjロール設定", description="SNSジャパン購入時に付与するロールを設定します")
    @is_allowed()
    @app_commands.autocomplete(vm_id=_vm_id_ac)
    ...
    @app_commands.command(name="snsjロール解除", description="SNSジャパン購入時のロール付与設定を解除します")
    @is_allowed()
    @app_commands.autocomplete(vm_id=_vm_id_ac)
    ...
    @app_commands.command(name="snsjバランス確認", description="SNSジャパンアカウントの残高を確認します")
    @is_allowed()
    async def snsj_balance(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        result = await snsj_get_balance()
        embed  = discord.Embed(title="💰 SNSジャパン残高", color=discord.Color.gold(), timestamp=discord.utils.utcnow())
        embed.add_field(name="残高", value=f"```{result.get('balance', '取得失敗')} {result.get('currency', '')}```")
        embed.set_footer(text="SNSジャパン自販機")
        await interaction.followup.send(embed=embed, ephemeral=True)

    @app_commands.command(name="snsjサービス検索", description="SNSジャパンから利用可能なサービスをキーワード検索します")
    @is_allowed()
    @app_commands.describe(keyword="キーワード（例: Instagram Followers）")
    async def snsj_search_services(self, interaction: discord.Interaction, keyword: str = ""):
        await interaction.response.defer(ephemeral=True)
        services = await snsj_get_services()
        if not services:
        ...
        embed = discord.Embed(
            title=f"🔍 SNSジャパン検索: {keyword or '全件（先頭10件）'}",
            color=discord.Color.blurple(), timestamp=discord.utils.utcnow(),
        )
        embed.set_footer(text="サービスIDを /snsjサービス追加 で登録 | SNSジャパン自販機")
        for svc in filtered:
            embed.add_field(
    ...
    @app_commands.command(name="snsj売上集計", description="SNSジャパン自販機の売上・注文統計を表示します")
    @is_allowed()
    @app_commands.autocomplete(vm_id=_vm_id_ac)
    ...
    @app_commands.command(name="snsj_paypay登録", description="SNSジャパン自販機用PayPayアカウントを登録します")
    @is_allowed()
    ...
    async def paypay_register(self, interaction: discord.Interaction, phone: str, password: str):
        ...
        if "otp_reference_id" in result:
            pending = load_json(SNSJ_PENDING_FILE)
            ...
            save_json(SNSJ_PENDING_FILE, pending)
            await interaction.followup.send(
                embed=discord.Embed(
                    title="📱 SMS認証コードを入力してください",
                    description="SMSに届いた**4桁の認証コード**を `/snsj_paypay認証 コード` で入力してください。",
    ...
    @app_commands.command(name="snsj_paypay認証", description="SMSに届いた認証コードを入力します（SNSジャパン自販機用）")
    @is_allowed()
    ...
    async def paypay_otp(self, interaction: discord.Interaction, code: str):
        ...
        if not info:
            return await interaction.followup.send(
                "❌ 認証待ちデータがありません。先に `/snsj_paypay登録` を実行してください。", ephemeral=True
            )
        ...
        if result == "OK":
            ...
            await interaction.followup.send(
                embed=discord.Embed(title="❌ 認証エラー", description="コードが正しくないか期限切れです。\n`/snsj_paypay登録` からやり直してください。", color=discord.Color.red()),
                ephemeral=True,
            )

    @app_commands.command(name="snsj_paypay確認", description="登録済みのPayPayアカウントを確認します（SNSジャパン自販機用）")
    @is_allowed()
    async def paypay_check(self, interaction: discord.Interaction):
        info = load_paypay_data().get(str(interaction.user.id))
        if not info:
            return await interaction.response.send_message("❌ PayPayアカウントが登録されていません。`/snsj_paypay登録` で登録してください。", ephemeral=True)
        embed = discord.Embed(title="✅ PayPay登録情報", color=discord.Color.green())
        embed.add_field(name="電話番号", value=f"```{info['phone']}```", inline=True)
        embed.set_footer(text="SNSジャパン自販機")
        await interaction.response.send_message(embed=embed, ephemeral=True)

    @app_commands.command(name="snsj自動メニュー追加", description="SNSジャパンサービス一覧から利益マージンを付けてまとめて追加します")
    @is_allowed()
    @app_commands.autocomplete(vm_id=_vm_id_ac)
    @app_commands.describe(
        vm_id="追加先の自販機ID",
    ...
    async def snsj_auto_add_menu(
        ...
        services = await snsj_get_services()
        if not services:
            return await interaction.followup.send("❌ サービスの取得に失敗しました。APIキーを確認してください。", ephemeral=True)
        ...
        for tmpl in DEFAULT_MENU_TEMPLATES:
            if kw_filter and kw_filter not in tmpl["keyword"] and kw_filter not in tmpl["name"].lower():
                continue
            svc = _match_service(services, tmpl.get("keywords", tmpl["keyword"]))
            if not svc:
        ...
        embed.add_field(
            name="反映方法",
            value="`/snsjパネル設置` でパネルを再設置すると反映されます。",
            inline=False,
        )
        embed.set_footer(text=f"マージン倍率: {margin_rate}x | SNSジャパン自販機")
        await interaction.followup.send(embed=embed, ephemeral=True)

    @app_commands.command(name="snsj利益確認", description="登録済みサービスの原価・販売価格・利益率を一覧表示します")
    @is_allowed()
    @app_commands.autocomplete(vm_id=_vm_id_ac)
    @app_commands.describe(vm_id="自販機（省略で全件）")
    async def snsj_profit_check(self, interaction: discord.Interaction, vm_id: str = None):
        menu = load_snsj_menu()
        if vm_id:
            menu = [m for m in menu if m.get("vm_id") == vm_id]
        menu = [m for m in menu if m.get("owner_id") == str(interaction.user.id)]
        ...
        for m in menu:
            ...
            if cost_rate:
                cost_jpy = round(float(cost_rate) * SNSJ_RATE_UNIT / SNSJ_RATE_PER, 4)
...
async def setup(bot: commands.Bot):
    await bot.add_cog(SnsjVendingCog(bot))
