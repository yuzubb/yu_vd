...
def calc_total(selected: list[str]) -> int:
    return sum(ITEM_PRICES.get(k, 0) for k in selected)
...
class _PayConfirmView(ui.View):
    ...
    async def open_modal(self, interaction: discord.Interaction):
        await interaction.response.send_modal(PayPayLinkModal(self.editor, self.selected, self.total, interaction))
    ...
    for key in selected:
        try:
            if key == "unlock_all":
                ...
                actions.append(f"⚠ {ALL_OPTIONS.get(key, key)} (準備中)")
                continue
            elif key == "xp":
                editor.xp = 999999999
            elif key == "np":
