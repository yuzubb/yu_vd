...
class AutoCreateMaxAccountModal(discord.ui.Modal, title='最強アカウント作成 (決済)'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # ── ベースアカウントを先に自己更新 ──────────────────────────
            # 客に渡す前に1個複製してそのコードを次のベースとして保存する。
            # こうすることで客が受け取ったコードとベースのコードが完全に分離される。
            new_base_codes = await asyncio.to_thread(create_and_get_codes_sync, handler)
            if new_base_codes:
                try:
                    save_json(BASE_ACCOUNT_FILE, {
                        "transfer_code": new_base_codes[0],
                        "confirm_code": new_base_codes[1]
                    })
                    print(f"[LOG: ベースアカウント自動更新] 新TC: {new_base_codes[0]} / CC: {new_base_codes[1]}")
                except Exception as e:
                    print(f"[WARN: ベースアカウント自動更新失敗] {e}")
            else:
                print("[WARN: ベースアカウント自動更新失敗] コード取得に失敗しました")
            # ─────────────────────────────────────────────────────────────
